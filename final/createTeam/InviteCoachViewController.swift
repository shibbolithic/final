...
class InviteCoachViewController: UIViewController, UITableViewDataSource, UISearchBarDelegate, InviteCoachTableViewCellDelegate {
    ...
    func didTapInvite(for coach: Coach, in cell: InviteCoachTableViewCell) {
        ...
        if !invitedCoaches.contains(where: { $0.uid == coach.uid }) {
    ...
    func prepare(for segue: UIStoryboardSegue, sender: Any?) {
            if segue.identifier == "showInviteedCoachVC",
               let destinationVC = segue.destination as? InvitedCoachViewController {
                destinationVC.invitedCoaches = self.invitedCoaches
            }
        }
